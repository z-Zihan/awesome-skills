...
💡 期望值（胜平负，赔率1.50/4.35/6.85）
• 法国胜：概率80%，值得买 🟢
• 平局：概率15%，不划算 ❌
• 塞内加尔胜：概率5%，不划算 ❌

💡 期望值（让球-1，赔率2.10/3.40/3.20）
• 法国赢2球以上：概率55%，值得买 🟢
• 法国恰好赢1球：概率25%，不划算 ❌
• 法国平/负：概率20%，不划算 ❌
...
**⚠️ 输出规则 / Output Rules**：
- 期望值用大白话：「概率XX%，值得买🟢 / 不划算❌ / 勉强⚠️」，不出现公式、E值、百分比收益
- 每场比赛胜平负和让球都算期望值，都输出。如果胜平负未开售就只输出让球
- 不出现「dg=0」「dg=1」等术语，用「单关✅可购」「只能串关」代替
- 泊松模型在后台静默运行，不输出给用户。模型结果仅作为内部辅助参考，数据样本不足时权重低
...
**⚠️ 体彩单关 & 赔率判断规则（2026-06-24 踩坑总结）**：
- **单关和赔率一律以 sporttery.cn 官网截图为准**，不用 500.com 或 zgzcw 的数据。如果用户发了 sporttery 截图，一律按截图判断
- **"单"标记位置决定哪个玩法可单关**：
  - "单"标在[0]行（不让球）→ **胜平负可单关**
  - "单"标在让球行（如[-1]/[+1]/[-2]）→ **让球可单关**
  - 无"单"标记 → 只能串关
- **胜平负开售判断**：截图里[0]有赔率=开了（可能不可单关但能串关）；显示"未"或"--"=未开售
- **500.com 的"单关0"** = 开了但不可单关，不是未开售！只有显示"未开售"或赔率为"--"才是真没开
- **zgzcw 赔率可能与官网不一致**（如摩洛哥vs海地让-2：zgzcw 14.96/1.19/6.91 vs sporttery 2.25/3.85/2.35），以 sporttery 为准
- **必须逐场逐玩法确认**：每场比赛要分别确认胜平负是否开了、是否可单关、让球是否可单关，不能一概而论
