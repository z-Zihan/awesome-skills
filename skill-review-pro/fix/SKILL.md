...
### Step 6：修复后回归评审 / Post-Fix Re-review

修复完成后，询问用户：

> "修复已完成。是否需要重新评审一遍验证修复效果？"

**触发词**："再评一遍"、"re-review"、"再测一下"、"再评"

用户确认后：
1. 以修复后的 Skill 文件为输入，重新执行 Phase 1（静态审查）
2. **自动带入历史分数**，在报告中生成对比：
...
3. 如果首次评审执行过 Phase 2（测试），询问是否也重新执行 Phase 2
4. 如果用户不确认，流程结束

**注意**：回归评审使用与首次评审相同的域权重和执行模式。
