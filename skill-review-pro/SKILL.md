...
## Benchmark 模式 / Benchmark Mode

**⚠️ 轻量模式，仅供稳定性验证，轻易不要触发。**

### 触发条件

用户明确说"稳定性测试"、"benchmark"、"跑几轮"、"测一下稳定性"。

### 执行规则

1. 以当前被测 Skill 为输入，**连续执行 3-5 轮 Phase 1**（仅静态审查，不执行 Phase 2）
2. **每轮之间重新读取 Skill 文件**，不缓存上轮结果
3. 每轮独立评分，使用相同的域权重
...
| 维度 | R1 | R2 | R3 | R4 | R5 | 均值 | 标准差 |
|------|----|----|----|----|----|------|--------|
| Reliability | X | X | X | X | X | X.X | X.X |
| Engineering | X | X | X | X | X | X.X | X.X |
| UX | X | X | X | X | X | X.X | X.X |
| Maintainability | X | X | X | X | X | X.X | X.X |
| **总分** | **XX** | **XX** | **XX** | **XX** | **XX** | **XX.X** | **X.X** |

**稳定性评估**：
- 标准差 < 3：✅ 评分稳定，评审结论可信
- 标准差 3-6：⚠️ 有波动，建议关注高分和低分的具体差异原因
- 标准差 > 6：❌ 评分不稳定，评审结论不可信，需要优化评审标准

**波动最大的维度**：<维度名>（标准差 X.X）
**波动原因分析**：1-2 句简要说明（如"该维度判断标准有模糊处"）
```
