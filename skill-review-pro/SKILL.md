...
- 总分和等级，格式如下（必须是报告正文第一行，使用 H2 标题）：

  ## 🏅 73 分 — ⚠️ 合格

  语言跟随用户：用户用中文则显示中文等级名，用户用英文则显示英文等级名，不同时显示两种语言。等级图标和名称见 `scoring/SKILL.md` 评分等级表。
- 执行模式和失败归因分布
- Top 3 优点
- Top 3 改进优先级
- 详细改进建议
...
**流程**：
1. 快速静态审查（只跑 Phase 1，不跑 Phase 2，不输出完整报告）
...
**目标 Skill**：xxx
**修复前快速评分**：R=XX / E=XX / UX=XX / M=XX → XX 分
...
**注意**：此模式不输出完整评审报告，只输出修复报告。如需完整评审，修复完成后可说「再评一遍」触发回归评审。

### 意见验证模式 / Opinion Validation Mode（用户提供修复意见时触发）

用户拿着修复意见（自己写的或别人给的），说"按这个改"时，先验证意见有效性。

**触发词**：「验证一下」/「这个改法对吗」/「帮我看看这几条建议」/「validate」

**前置条件**：用户提供了目标 Skill + 修复意见

**流程**：
1. Phase 1 静态审查（先独立评审 Skill，不看用户意见）
2. 逐条验证用户的修复意见（对照评审结论，不盲从）
...
|------|------|
| ✅ 有效 | 确实是问题，修法合理，建议执行 |
| ⚠️ 有效但不完整 | 方向对但修法不够，给出补充建议 |
| 🔄 可选 | 不是问题，是风格偏好，用户自决 |
| ❌ 无效 | 不是问题，或修法会引入新问题，说明原因 |
| ➕ 遗漏 | 用户意见没覆盖到的真实问题，额外列出 |
...
|---|------------|------|------|
| 1 | ... | ✅ 有效 | 确实存在，修法合理 |
| 2 | ... | ❌ 无效 | 该部分逻辑正确，修改反而引入歧义 |
| 3 | ... | ⚠️ 有效但不完整 | 方向对，但还需同步修改 XX 章节 |
...
4. ⏸ 询问用户：「是否继续执行有效的修复？」
   - 是 → 进入 `fix/SKILL.md`，只执行「✅ 有效」和「⚠️ 有效但不完整」的条目
   - 否 → 结束

**注意**：意见验证是独立判断，不是用户的橡皮图章。如果用户的修复意见整体质量低（多数无效），在报告中明确指出，不要为了面子通过无效修改。
