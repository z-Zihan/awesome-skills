...
| TESTED_POINTS_COOLDOWN_TESTS | 10 | 知识点冷却期：最近10次测评内考过的知识点禁止再出 |
...
11. **知识点冷却**：最近10次测评考过的知识点禁止再出，避免反复考同一考点
...
10. **考点冷却追踪**：最近10次测评考过的知识点禁止再出，搜题/AI出题均须排除已考点；同一试卷内知识点不重复
...
- **冷却范围**：最近 10 次测评考过的知识点，本次测评禁止出现
- **跨试卷去重**：同一份试卷内知识点不重复（已有规则），冷却期机制保证跨试卷也不重复
- **搜索时排除**：搜到真题后，先检查题目的知识点是否在冷却期内（point 字段匹配），在冷却期内 → 跳过该题，搜其他题
- **AI出题排除**：AI 出题前读取 `tested_points.json`，排除冷却期内的知识点再出题
- **子考点轮换**：薄弱项侧重增加某维度出题量时，不得反复考同一子考点，必须在维度的子考点池中轮换（如语法弱 → 本次出倒装句，下次出独立主格，不是每次都出虚拟语气）
- **冷却期耗尽时的回退**：如果某维度所有常见子考点都在冷却期内（极少发生），优先从冷却期最远的子考点中选题，或适当放宽子考点粒度（如从"虚拟语气-与过去事实相反"放宽为"虚拟语气-与将来事实相反"）
...
5. **知识点多样性（强制，多途径扩题库 + 冷却期追踪）**：
   - **禁止重复考点**：同一测评中不出现重复考点；跨测评通过 `tested_points.json` 冷却期强制去重（最近10次测评考过的知识点禁止再出）
   - **搜题排除已考点**：搜到真题后，先检查题目的知识点是否在 `tested_points.json` 冷却期内，在冷却期内 → 跳过该题，继续搜其他题
   - **AI出题排除已考点**：AI 出题前读取 `tested_points.json`，排除冷却期内的知识点再出题
...
搜题失败时：静默回退AI自身知识出题，不提示用户，但必须避免与最近 10 次测评的知识点重复（读取 `tested_points.json` 冷却期检查）。AI 出题也必须保证随机性和多样性（见通用规则第5条）
...
- **历史去重**：出题前读取 `tested_points.json`，检查最近 10 次测评的考点，确保本次搜到的题目不与近期重复。若搜到的真题已被用过，换年份/套号重新搜
