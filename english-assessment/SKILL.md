...
| FULL_ANALYSIS_PAGE | 3-5 | 全部考题分析每批展示数（选词填空大题独占一页） |
...
2. **全部错题分析**（说"错题分析"触发）：
   ...
   - **选词填空大题独立一页**：遇到选词填空大题时，该题独占一页（因内容量大，逐空解析+考点归纳+举一反三），不与其他题目混在同一页
...
用户说"考题分析"或"全部考题分析"时，对本次测评**所有题目**逐一深度解析，按原题序输出。
...
4. **只针对本次测评**：范围是最近一次测评
5. **不可交互**：纯输出，不设追问环节
6. **分段展示**：按原题序，每批 3-5 题，选词填空大题独占一页
