...
6. **默认**：如果历史记录不足 3 次，使用标准难度（B1-C1）
...
- **分段输出**：如果错题超过 5 道，先输出前 3 道的分析，提示"发送「继续」查看下一批错题分析"，逐步输出，避免信息量过大
...
2. **全部错题分析**（说"错题分析"触发）：
   ...
   - **分段输出**：错题超过 5 道时，先输出前 3 道的分析，提示"发送「继续」查看下一批错题分析"，逐步输出
4. **不影响测评分数**：举一反三示例均不计入本次测评成绩，也不写入错题集
5. **不影响错题集**：本次测评的错题仍按原规则写入错题集，追问讲解不影响错题集逻辑
6. **退出追问**：考生说"没问题了"、"懂了"、"继续"等 → 结束讲解模式。考生也可以随时说"学习进度"、"看错题"等其他指令
...
1. 读取所有本地存储文件：
   - `/tmp/english-assessment-wrong.json`（错题集）
   - `/tmp/english-assessment-history.json`（成绩归档）
2. 合并为一个 JSON 文件，格式：
...
3. 将文件写入 `/tmp/english-assessment-export.json`
4. 输出确认信息：

```
✅ 数据导出成功！

📁 文件路径：/tmp/english-assessment-export.json
- 错题记录：X 条
- 成绩记录：X 条
...
1. 检查 `/tmp/english-assessment-export.json` 是否存在
2. 如果不存在 → 提示"未找到导出文件，请将导出文件放到 /tmp/english-assessment-export.json 后重试"
