...
- 用户说"跳过" → 该题记0分，出下一题
- 用户说"暂停测评" → 保存当前进度（已完成题目记录、当前得分、剩余题数）到 `~/.openclaw-autoclaw/workspace/english-assessment/paused-session.json`，提示"测评已暂停，发送「继续测评」恢复"。恢复后从暂停的下一题继续，不重新出题
...
2. **内容范围**：
   ...
   - 中译英翻译题中各类场景均衡覆盖（日常、学术、职场、社会等），不限定单一场景
...
7. **跨 session 保持**：测评结束后，将本次测评的错题摘要（题目、用户答案、正确答案、得分、题型、维度）临时存入 `~/.openclaw-autoclaw/workspace/english-assessment/latest-session.json`，供错题追问时读取。考生说"错题分析"或追问某题时，从该文件获取本次测评上下文，不依赖 AI 内存。新测评开始时覆盖此文件
