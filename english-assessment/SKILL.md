...
⚠️ 出题硬约束：
1. **选词填空大题必出**：默认测评和快速测评都必须包含1道选词填空大题（8选5，5分），独立板块
...
回复格式：1-A 2-F 3-C 4-E 5-H（也可逐空回答）
...
3. **大题板块**：默认测评中**必须包含1道选词填空大题**（8选5，在总题数中计为1题），占5分，作为独立板块出现
4. **分值分配**：
...
3. **大题板块**：快速测评中也**包含1道选词填空大题**（8选5，在总题数中计为1题），占5分
4. **分值分配**：
   - 客观题：单题3-5分
   - 半主观题：单题5-7分
   - 主观题：单题8-12分
...
8. **选词填空大题出题规则**：
   ...
   - 8个备选词：覆盖名词、动词、形容词、副词四大词性，每个词性至少1个
   - 5个空格：需兼顾不同词性考点，空格分布均匀
   - 3个干扰项：词性或语义与正确答案相近，具有合理干扰性
   - **真题截取策略**：搜到的真题原文如果有10个空格，只截取包含5个连续空格的段落（前半段或后半段），确保截取部分语义相对完整、句子不断裂。从原文10个正确答案中选截取部分对应的5个，再加3个干扰项（可从原文剩余5个正确答案中选，或AI补充），组成8个备选词
   - AI自主出题时：直接按8选5格式设计，文章可更短（100-150词）
   - 优先联网搜索CET-4近5年真题选词填空原文，搜不到时静默回退AI自主出题，不提示用户
   - **考试形式**：
     - 先展示完整短文（空格处标数字编号1-5）和8个备选词（标号A-H，含词性标注）
     - 考生回复填空答案，格式如"1-A 2-F 3-C 4-E 5-H"
     - 也支持逐空作答：考生只回答一个空格编号对应字母，AI记录后提示下一个未答空格
     - 混合作答时，以用户最后提交的答案为准；逐空作答时 AI 追踪已答空格编号，跳过已答空格
     - **所有5个空格答完后统一判分**，静默记录，不做中间反馈
   - **评分**：每空1分，共5分，完全正确得1分，错误0分
...
| 8.5 | 选词填空大题 | 8选5：给一篇短文（100-200词）+8个备选词（含3个干扰项）+5个空格，考生填写对应编号，一词只用一次 | 客观（每空1分，共5分） |
