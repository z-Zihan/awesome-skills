...
数据文件：./english-assessment/wrong.json + history.json + export.json + tested_points.json
...
- **换出法示例**：虚拟语气第1次出"If I ____ (know)"（填空），第4次可再出虚拟语气但换为"I wish I ____ (be)"或选择题形式——同一考点，不同考查方式
...
- **与冷却期的协同**：冷却期看的是"最近10次测评"（看次数），清理看的是"超过10天"（看时间）。两个条件独立判断：知识点必须同时不在最近10次测评内 AND 不在最近10天的记录中，才能重新出现

### 写入时机

- 每道题出完后，立即追加该题的知识点记录到 `tested_points.json`
...
5. **备选词标注词性**：原真题不标注词性，但加上词性标注能帮考生排除，8选5的备选词必须加 `(n.)` `(v.)` `(adj.)` `(adv.)` 标注
...
- 全部答完 → **评分静默复核**：对本次测评所有评分静默复核一遍——检查有无错判（答对判错）、误判（扣分不合理）、漏判（遗漏评分项）。发现问题则静默修正。复核完成后输出测评报告 + 更新错题集 + 写入成绩归档（含 started_at 和 finished_at）+ 清理 tested_points.json（删除超过10天的记录）
- 中途"退出测评" → 按已完成题目输出报告 + 更新已完成题目的错题集 + 写入成绩归档 + 清理 tested_points.json。得分永远按百分制展示（得分 = 实际得分 ÷ 实际满分 × 100，四舍五入取整），不显示原始分/原始满分。题量过少时在报告末尾强调"题量过少，分数不具参考价值"
- 用户说"跳过" → 该题记0分，出下一题
...
5. **知识点多样性（强制，多途径扩题库 + 冷却期追踪）**：
   - **禁止重复考点**：同一测评中不出现重复考点；跨测评通过 `tested_points.json` 冷却期追踪——词汇15次完全禁重复，其他维度3次冷却后可换出法重复（10次内≤2次）
   - **换出法规则**：同一知识点再次出现时，必须换考查方式（如填空→选择题，或换句型/语境），不得原题复现
   - **搜题排除已考点**：搜到真题后，先检查题目的知识点是否在冷却期内，词汇→完全跳过；其他维度→已达2次上限则跳过，未达上限且能换出法则保留
   - **AI出题排除已考点**：AI 出题前读取 `tested_points.json`，按同样规则排除
   - **单份试卷≤30%重复**：单份试卷中最多30%的题目可涉及近期已考知识点，同一知识点在同一份试卷内仍不出现两次
   ...
   - **AI 出题随机性**：AI 出题时也必须保证随机性和多样性——
     - 词汇：从不重复的领域词库中随机选题（科技/医学/法律/金融/文学等轮换），避免反复出同类词
     - 语法：从 20+ 语法考点中随机选题，不连续出同类型语法题
     - 翻译：从不重复的话题池中随机选句（文化/经济/社会/科技等轮换）
     - 阅读：从不重复的文章主题中随机选材
...
搜题失败时：静默回退AI自身知识出题，不提示用户，但必须避免与冷却期内的知识点重复（词汇15次完全禁重复，其他维度3次冷却后可换出法重复，见「知识点追踪」冷却期规则）。AI 出题也必须保证随机性和多样性（见通用规则第5条）
...
- **历史去重**：出题前读取 `tested_points.json`，按分维度冷却规则检查（词汇15次禁重复，其他3次冷却+10次内≤2次），确保本次搜到的题目不与近期重复。若搜到的真题已被用过，换年份/套号重新搜
