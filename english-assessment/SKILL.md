...
description: >
  ...
  内容覆盖各专业领域。
  触发词：开始英语测评 / 英语测试 / 测一下英语 / 英语水平测评 / 快速测评 / 错题重测 / 看错题 / 学习进度 / 进步曲线 / 导出数据 / 导入数据
...
10. **陪伴成长**：不是一次性工具，而是长期学习伙伴，每次测评都在为你量身定制
9. **数据可迁移**：支持导出/导入全部本地数据，换电脑不丢进度
...
3. **提难度方式**：
   - 词汇题：使用更专业/生僻的词汇和熟词生义，从 B2-C2 提升到 C1-C2+，增加习语和短语动词
   - 语法题：增加高阶语法点（虚拟语气多种用法、倒装句全部形式、强调句、独立主格、省略与替代、非谓语动词复合结构），难度对标 C1-C2
   - 翻译题：增加长句翻译、专业术语翻译
   - 阅读理解：文章加长至 300-500 词，增加深层推理题、作者态度题、段落关系题，难度对标 C1-C2
   - 实用表达：增加正式场景、跨文化沟通、学术/职场等多样化表达题目
...
2. **内容范围**：
   ...
   - 词汇题强制联网搜题：优先从雅思/托福/GRE/GMAT/专四专八/考研英语等真题词库和专业术语库中获取，确保词汇新鲜多样，避免 AI 反复出相同词汇
   ...
   - 联网搜题失败时：才使用 AI 自身知识出题，但必须避免与最近 3 次测评的知识点重复
3. **难度**：CEFR B1-C1（标准难度），受自适应难度机制调整。基础题约占20%，中高级题约占80%。语法题标准难度对标雅思7+ / 专四专八，涵盖复合句、虚拟语气、倒装、强调句、独立主格等
4. **知识点多样性（强制）**：
   - **禁止重复考点**：同一测评中不出现重复考点；跨测评也必须尽量扩大题目多样性
   - **联网搜题**：每次出题前，**必须优先联网搜索**（使用 web_search 或 web_fetch）获取真实英语考试题目、专业术语、时事话题，确保知识点新鲜多样，避免 AI 凭自身知识反复出相似题目
   - **工具可用性检查**：如果联网工具不可用（模型无 web_search/web_fetch），直接用 AI 知识出题，并在第一题前提示"⚠️ 当前无法联网搜题，题目基于 AI 知识生成，知识点多样性可能受限"
   - **搜题策略**：
     - 搜索真实考试题源：雅思/托福/GRE/GMAT/专四专八/CPE 等真题或模拟题
     - 搜索专业领域最新术语和表达（科技、医学、法律、金融等）
     - 搜索时事热点相关英语表达，确保内容与时俱进
     - **搜题失败时**：才使用 AI 自身知识出题，但必须避免与最近 3 次测评的知识点重复
...
7. **选词填空大题出题规则**：
   - **难度来源**：60% CET-4近5年真题，30% CET-6近5年真题，10%其他同等难度真题，优先使用真题原文，联网搜索获取
   ...
   - 优先联网搜索CET-4近5年真题选词填空原文，搜不到时AI自主出题
   - **考试形式**：
...
| 5 | 中译英翻译 | 翻译中文为英文（60% CET-4真题 + 30% CET-6真题 + 10%同等难度真题） | 主观 |
...
💬 想了解错题的详细分析吗？发送「错题分析」查看本次所有错题的讲解和举一反三，也可以针对某道题追问（如"第3题为什么错？"）
```
...
{
  "export_version": "1.0",
  "exported_at": "2026-06-02T22:00:00+08:00",
  "wrong_records": { ... },
  "history_records": { ... }
}
```

3. 将文件写入 `./english-assessment/export.json`
4. 输出确认信息：

```
✅ 数据导出成功！

📁 文件路径：./english-assessment/export.json
- 错题记录：X 条
- 成绩记录：X 条

💡 将此文件复制到新电脑，然后发送「导入数据」即可恢复进度
```

5. 如果没有任何数据 → 提示"暂无数据可导出，先完成一次测评吧"

### 导入数据

用户说"导入数据"时：

1. 检查 `./english-assessment/export.json` 是否存在
2. 如果不存在 → 提示"未找到导出文件，请将导出文件放到 ./english-assessment/export.json 后重试"
3. 读取并验证文件格式（必须有 export_version 字段）
4. 格式不合法 → 提示"导出文件格式不正确，请确认文件完整性"
5. JSON 解析失败（文件损坏、截断、乱码）→ 提示"导出文件可能已损坏，请检查文件是否完整或重新导出"
5. 格式正确 → 执行合并：
   - **错题集**：与现有记录合并，按 id 去重（相同 id 保留较新的记录）
   - **成绩归档**：与现有记录合并，按 date 去重（相同时间戳的记录不重复添加）
6. 合并完成后，应用清理规则（数量上限、时间上限等）
7. 输出确认信息：
...
📊 导入结果：
- 错题记录：原有 X 条 + 导入 Y 条 = 合并后 Z 条
- 成绩记录：原有 X 条 + 导入 Y 条 = 合并后 Z 条

💡 发送「学习进度」查看你的完整进度
```

8. **冲突处理**：如果本地已有数据，提示"检测到本地已有数据，将进行合并（去重保留较新记录），不会覆盖现有数据"
...
- 联网搜题为强制性要求，优先从真实考试题源和专业资源获取题目，确保知识点新鲜多样
