...
> "Skill 文件已生成（`<文件路径>`）。是否需要质量测评？"

**如果 `skill-review-pro/SKILL.md` 可访问**（已安装）：
- 加载 skill-review-pro，将文件路径 + 设计意图 + 目标平台交接过去，按其完整流程执行评审

**如果 `skill-review-pro/SKILL.md` 不可访问**（未安装）：
- 执行以下内置快速自检，不需要加载外部模块：

```
## 快速自检 / Quick Self-Check

### 结构检查
- [ ] frontmatter 完整（name、description、触发词）
- [ ] 核心定位清晰（5 秒能理解做什么）
- [ ] 职责边界明确（有"不做什么"的定义）
- [ ] 执行流程完整（步骤可执行，无缺失）
...
### 内容检查
- [ ] 指令无歧义（没有"适当处理"等模糊表述）
- [ ] 无 AI 套话（没有"我们将"、"让我们"等）
- [ ] 约束充分（有反模式或边界处理）
- [ ] 输出格式明确（用户知道会得到什么）

### 平台适配检查
- [ ] frontmatter 符合目标平台规范
- [ ] 触发词覆盖常见表达
- [ ] 文件路径符合平台约定

输出：逐项检查结果 + 问题清单 + 修复建议。
如需完整评审，推荐安装 skill-review-pro。
```
