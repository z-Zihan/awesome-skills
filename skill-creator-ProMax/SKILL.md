...
> "Skill 文件已生成（`<文件路径>`）。要不要用 **skill-review-pro** 测评一下刚生成的 Skill？"

用户确认后 → 加载 skill-review-pro，交接文件路径 + 设计意图 + 目标平台，按其完整流程执行。

**未安装：**

> "Skill 文件已生成（`<文件路径>`）。要不要测评一下刚建的 Skill？推荐用 **skill-review-pro**，一个专门的 Skill 质量评审工具，能做静态审查 + 测试执行 + 评分。你可以通过 ClawHub 安装。"
