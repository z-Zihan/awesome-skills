...
- **报告语言跟随用户**：用户用中文则全中文输出，用英文则全英文输出。不在同一行或同一个单元格中混用中英文。技术术语（API、diff、PR 等）保留英文原文即可
...
> Major/Critical 的问题描述必须包含影响链（**改动** → **影响** → **级联**），不允许只写一句结论。

| # | 严重度 | 置信度 | 位置 | 问题描述 | 修复建议 |
|---|--------|--------|------|----------|----------|
| 1 | Critical | HIGH | 文件:函数:行号 | **改动**：具体改了什么 → **影响**：直接后果 → **级联**：最坏场景 | 修复方向 |
| 2 | Major | HIGH | 文件:函数 | **改动**：... → **影响**：... | 修复方向 |
| 3 | Minor | MEDIUM | 文件:函数 | 简要描述即可 | 修复方向 |
